...
- `src/server.ts` (built as the `server` environment) parses appsettings, boots a middleware-mode Vite dev server when not in production, composes the four request handlers into an h3 app, and listens via `listhen` (using the Vite HTTPS config in dev). It also wires the OTel-adapted `PhoriaLogger` and graceful SIGTERM/SIGINT shutdown.
