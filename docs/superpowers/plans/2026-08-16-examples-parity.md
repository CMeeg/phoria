...
- [x] **Step 2: Verify standalone consumption.** Fetch each committed example with giget into a temporary directory, run standalone `pnpm install`, and run `pnpm build`. — ✅ PASS, 9/9 (re-run 2026-09-25 after the `canary` merge, against published `0.5.0-beta.1`): framework-multiple, framework-react, framework-svelte, framework-vue, getting-started, with-storybook, with-styled-components, with-tailwind, with-workspace all fetch, install with `--frozen-lockfile`, and build standalone. The earlier "blocked pre-merge" note was a **wrong diagnosis**: the unqualified path `gh:CMeeg/phoria/examples/<name>` resolves the *default* branch, and `examples/` is still absent from `main` (PR #36 open), so the 404 was a branch mismatch, not an unpublished tree. The working form is `npx giget@latest gh:CMeeg/phoria/examples/<name>#canary` — `#` for a ref, not `@` (that is npm dist-tag syntax). All nine READMEs and `examples/README.md` still document the unqualified form, which will keep 404ing until PR #36 merges.
- [x] **Step 3: Run unrestricted local e2e.** Run `pnpm examples:e2e` and confirm all nine examples execute sequentially. — ✅ PASS, 9/9 (re-run 2026-09-25, unrestricted, exit 0): framework-multiple 5, framework-react 4, framework-svelte 4, framework-vue 4, getting-started 3, with-storybook 4, with-styled-components 5, with-tailwind 5, with-workspace 3. The earlier 8/9 with `with-workspace` failing `emits modulepreload directives` was the registry version skew against `0.5.0-beta.0`; it is resolved now that PR #39 synced the examples to `0.5.0-beta.1`. with-styled-components and with-tailwind had additionally failed on a `react-counter` marker assertion, fixed per maintainer direction by asserting the server-rendered counter state (`count is 5`) instead.
- [x] **Step 4: Run the CI allow-list.** Run `EXAMPLES_E2E=getting-started,framework-multiple,with-workspace pnpm examples:e2e`. — ✅ PASS, 3/3 (re-run 2026-09-25, exit 0): getting-started 3, framework-multiple 5, with-workspace 3. The earlier 2/3 with `with-workspace` failing the same modulepreload assertion is resolved post-sync.
