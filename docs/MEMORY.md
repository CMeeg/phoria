...
- **`canary` replaces `develop`**: created at develop HEAD, `ci.yml` triggers move `develop` → `canary`, and feature PRs target canary; `main` gets only coordinated stable cuts. First betas are `0.5.0-beta.N` (the 16 pending minor/patch changesets), so consumers benefit from the phased v1 work as stable releases as it matures; the `1.0.0` major changesets stay queued for Phase 10.
...
- Rehearsed the Changesets prerelease flow in a throwaway worktree with the 16 pending changesets. `pnpm install` completed cleanly using pnpm 11.17.0.
- Beta versions produced by `pnpm changeset pre enter beta` followed by `pnpm run version`: `@phoria/phoria` -> `0.5.0-beta.0`, `@phoria/phoria-react` -> `1.0.0-beta.0`, `@phoria/phoria-svelte` -> `1.0.0-beta.0`, `@phoria/phoria-vue` -> `1.0.0-beta.0`, `@phoria/opentelemetry` -> `1.0.0-beta.0`, `@phoria/vite-plugin-dotnet-dev-certs` -> `0.3.0-beta.0`, and `phoria-dotnet` -> `0.5.0-beta.0`. The dotnet package version synchronized to `0.5.0-beta.0`.
- The beta peer rewrite for `@phoria/phoria` was exactly `>=0.5.0-beta.0` for `@phoria/phoria-react`, `@phoria/phoria-svelte`, `@phoria/phoria-vue`, and `@phoria/opentelemetry`; Changesets did not retain the prior `<1.0.0` upper bound.
- The generated `.changeset/pre.json` had `mode: "pre"`, `tag: "beta"`, `initialVersions` of `phoria-dotnet`/`@phoria/phoria`/`@phoria/phoria-react` = `0.4.2`, `@phoria/opentelemetry` = `0.1.0`, `@phoria/phoria-svelte`/`@phoria/phoria-vue` = `0.3.2`, and `@phoria/vite-plugin-dotnet-dev-certs` = `0.2.1`, plus these observed changeset IDs: `build-app-server-environment`, `decouple-h3-from-public-api`, `dotnet-10-xunit-v3`, `eager-rivers`, `graceful-server-process-stop`, `otel-observability-examples`, `quiet-island-styles-css-warning`, `quiet-otlp-requests`, `raise-node-and-pnpm`, `typescript-6-vite-plugin-dts-5`, `unavailable-server-policy`, `update-remaining-dependencies`, `vite-8-rolldown`, `vite-plugin-correctness`, and `widen-phoria-peer-range` (15 IDs observed despite the prerelease metadata describing 16 pending changesets).
- Stable-cut rehearsal with `pre enter beta`, `pre exit`, and `pnpm run version` removed `pre.json` cleanly and produced `@phoria/phoria` -> `0.5.0`, `@phoria/phoria-react` -> `0.5.0`, `@phoria/phoria-svelte` -> `0.4.0`, `@phoria/phoria-vue` -> `0.4.0`, `@phoria/opentelemetry` -> `0.2.0`, `@phoria/vite-plugin-dotnet-dev-certs` -> `0.3.0`, and `phoria-dotnet` -> `0.5.0`. Stable peers remained `>=0.4.0 <1.0.0` for all four peer packages. The rehearsal worktree was removed and pruned; the root worktree remained clean.
