...
if (viteDevServer) {
  ...
  // The published runtime derives the bundled entry from a `.ts` suffix; keep the source entry `.tsx` for Vite.
  const productionAppsettings = { ...appsettings, ssrEntry: appsettings.ssrEntry.replace(/\.tsx$/, ".ts") }
  app.use(createPhoriaSsrRequestHandler(productionAppsettings, { logger: phoriaLogger }))
}
